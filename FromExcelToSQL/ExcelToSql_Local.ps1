    ...
    try
    {
        ...
        {    
            ...
            {                
                ...
                $sqlcmd.CommandText =
@"
    IF NOT EXISTS (select 1 from SMS_ore_supply_0_300 where date=@date AND work_shift=@work_shift AND charge_number=@charge_number)
    BEGIN
...
"@; 
                    ...
                    $sqlcmd.Parameters["@date"].Value = ("{0}" -f [DateTime]::FromOADate($ArrayRow.'Дата')) 
            ...
            {   
                ...
                $sqlcmd.CommandText =
@"
    IF NOT EXISTS (select 1 from SMS_ore_output_0_10 where date=@date AND type=@type AND charge_number=@charge_number)
    BEGIN
...
$items = Get-ChildItem -Path $fff -Force
